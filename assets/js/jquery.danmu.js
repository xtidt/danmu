...
(function ($) {
    ...
    var Danmu = function (element, options) {
         ...
         * 计算行数
         * */
        var row_count = parseInt(heig / options.font_size_big);
            ...
            callback: function (index) {
                ...
                if ($(element).data("danmu_array")[$(element).data("nowtime")]) {
                    ...
                    for (var i = 0; i < danmus.length; i++) {
                        ...
                        $("#linshi").css({
                            "color": danmus[i].color, "-moz-opacity": $(element).data("opacity"), "opacity": $(element).data("opacity"), "white-space": "nowrap", "font-weight": "bold"
                        });
                        ...
                        * */
                        if (danmus[i].position == 0) {
                            ...
                            var row = parseInt(row_count * Math.random());
                            while (rows_used.indexOf(row) >= 0) {
                                var row = parseInt(row_count * Math.random());
                            }
                            rows_used.push(row);
                            //console.log(rows_used.length);
                            if (rows_used.length == row_count) {
                                rows_used = new Array();
                                row_count = parseInt(heig / options.font_size_big);
                            }
                            var top_local = (row) * options.font_size_big;

                            $("#linshi").css({"position": "absolute", "top": top_local, "left": options.width
                            });
                            var fly_tmp_name = "fly" + parseInt(heig * Math.random()).toString();
                            $("#linshi").attr("id", fly_tmp_name);//命名
                            //初始化滚动动画
                           /* $('#' + fly_tmp_name).animate({left: -$(this).width() * 1 }, danmus[i].duration || options.speed
                                , function () {
                                    $(this).remove();
                                }
                            );*/
                            $('#' + fly_tmp_name).animate({
                                ...
                                easing:danmus[i].easing || 'linear',
                                duration:danmus[i].duration || options.speed,
                                complete:function(){
                                    console.log(123);
                                    $(this).remove();
                                }
                             ...
                             * */
                            $('#' + top_tmp_name).css({
                                "width": options.width, "text-align": "center", "position": "absolute", "top": (5 + $(element).data("topspace"))
                            });
                            ...
                            $('#' + bottom_tmp_name).css({
                                "width": options.width, "text-align": "center", "position": "absolute", "bottom": 0 + $(element).data("bottomspace")
                            });
